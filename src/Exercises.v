...
Open Scope bool_map_scope.

(* 1. *)
Definition lte_or_gt: forall n m : nat, {n <= m} + {n > m}.
...
Eval compute in lte_or_gt 5 2.

Extraction lte_or_gt.
...
(* c *)
Definition truth_assns := bool_map.

Fixpoint propDenote (ts : truth_assns) (prop : prop) : Prop :=
  match prop with
    | PVar v => match ts v with
      | true => True
      | false => False
      end
    | PAnd lp rp => propDenote ts lp /\ propDenote ts rp
    | POr lp rp => propDenote ts lp \/ propDenote ts rp
    | PNeg p => ~propDenote ts p
  end.

Eval compute in propDenote(1 !-> true; empty_map)(PAnd (PVar 1) (PNeg (PVar 2))).
...
(* e *)
Definition decide (truth: truth_assns)(p: prop) : {propDenote truth p} + {~propDenote truth p}.
  induction p; crush.
  - destruct (truth v); crush.
Qed.

(* f *)
Definition negate : forall p, {p' : prop | forall truth, propDenote truth p <-> ~propDenote truth p'}.
  Hint Constructors prop : core.
  ...
  refine(fix F (p: prop) : {p' : prop | forall truth, propDenote truth p <-> ~propDenote truth p'} :=
    ...
    - destruct (truth v); auto.
    - apply (i truth) in H1; auto.
    - apply (i0 truth) in H2; auto.
    - apply (i truth) in H0; auto.
    - apply (i0 truth) in H0; auto.
    - destruct (decide truth neg_lp); auto.
Defined.

Eval compute in (negate (PAnd (PVar 1) (PVar 2))).

Eval compute in (negate (POr (PNeg (PVar 1)) (PVar 2))).
...
Definition formula : Set := list clause.

Definition ptruth_assns := pbool_map.

Fixpoint clauseDenote(truth: ptruth_assns)(cl: clause) : Prop :=
  match cl with
    | CVar v => match truth v with
      | Some true => True
      | _ => False
    end
    | CNeg cl => ~clauseDenote truth cl
...
Definition formulas := list clause.

Print andb.

Fixpoint clauseAssignedAndFalse(truth : ptruth_assns)(cl: clause) : Prop :=
  match cl with
    | CVar v => match truth v with
      | Some false => True
      | _ => False
    end
    | CNeg cl => clauseAssignedAndFalse truth cl
