...
Section ilist.
  ...
  (* Exercise, 5 minutes: Working with length-indexed lists *)

  (* Define a function irev that reverses an indexed list.  If you run into
     trouble, try defining a helper function first. *)
...
(* Exercise: 15 minutes.  Define a function izip that takes an ilist A and an
   ilist B of equal length and returns an ilist (A * B) containing the pairs of
...
(* Exercise 30 minutes : Define a 'safe' minus function on nats and fins.  The
   idea is that n - fin (S n) can not underflow. *)

(* Hint: There is a ~10 loc solution that involves matching on n first. Then you
   just need to show that you can recurse on the predecessor of fn.  Of course,
   there maybe be easier ways to define this.

   You may run into trouble proving that the recursion is guarded on n after
   matching on fn.  My solution was to use fn as the decreasing argument and use
   the 'convoy' pattern to include fn in the return clause.
 *)
...
(* Exercise: 5 mins. Define append for heterogeneous lists. *)





(* Exercise: 15 mins. Define map on heterogeneous lists.  Make sure you're
   allowing the resulting list to have a different argument 'A', the type of
   elements in the list indexing hlist, and not mapping from hlist A B ls ->
   hlist A B ls' *)
...
(* Exercise, 15 minutes : add Nats and Pairs. Then, add a let expression that
   will evaluate the expression in its second argument in an environment
   extended with the value of the first argument. *)
