...
(* Exercise, 20 minutes. Give an inductive definition for the type of a
   "perfect" binary tree indexed by depth. Then, define another inductive type
   that describes a path to any of the nodes in your tree (not just leaves).
   Finally, define a function that can safely retrieve the value associated with
   a node. *)


(* Exercise, 15 minutes. Give the recursive definitions of a perfect binary
   tree, a path to a node, and the function to get the value of a node. *)
...
(* The "reflexive" definition of length-indexed lists and the associated
   implementation of head, tail, cons, and map are left implicit in the
   definitons of rifoldr and sum above, and cfoldCond below. This makes the
   definitions using reflexive ilists unnecessarily verbose. It's possible to
   give explicit definitions for the common opperations on rilists, which makes
   using them slightly more natural: *)

Section rilist.

  Variable A : Type.

  Definition rilist (n : nat) : Type := ffin n -> A.

  Definition rnil : rilist 0 := fun n => match n with end.

  Definition rhead (n : nat) (l : rilist (S n)) : A := l None.

  Definition rtail (n : nat) (l : rilist (S n)) : rilist n := fun m => l (Some m).

  Definition rcons (n : nat) (a : A) (l : rilist n) : rilist (S n) :=
    fun m => match m with 
               | None => a 
               | Some m' => l m' 
             end.
...
(* Exercise : 15 minutes.  Give the "reflexive" definitions of heterogeneous
   lists along with head, tail and cons in the same style as above *)



(* Exercise : 10 minutes. Define app and map for reflexive heterogeneous lists.
   Note: make sure you use the simple "lazy" definition of map used in inc and
   sum, where the mapping function is not applied until an element in the list
   is accessed.  This is important for the last problem.

   Also, you can assume that your input and output hlists are indexed by the
   same list A. *)



(* Exercise : 25 minutes.  Rewrite the original (unmodified) stlc interpreter
   using the reflexive defintion of heterogeneous lists. This should only
   require changing type and constructor names in a couple of places.

   Then, add a 'variable arity' Let expression, that takes an arbitrary number
   of expressions, evaluates them in some environment s and then evaluates
   another expression in s extended by all of the resulting values.

   You might find the app and map functions you defined for reflexive hlists
   useful.
*)   
